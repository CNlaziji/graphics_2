...
// ============================================================================
// 成员 C：材质全局对象（在 InitializeResources 中初始化）
// ============================================================================
// 使用 std::optional 延迟构造，避免在 OpenGL 上下文创建前调用 GL 函数
#include <optional>
std::optional<Material> g_matWood;      ///< 木头材质
std::optional<Material> g_matStone;     ///< 石料材质
...
bool InitializeResources() {
    try {
        ...
        // [插槽 0] 成员 C - 纹理与材质初始化
        // ============================================================================
        // 使用 Material 工厂方法创建各类材质。
        // LoadTexture() 内置缓存，相同路径只上传一次显存。
        // 若纹理文件不存在，LoadTexture 会打印警告并返回 0，
        // Material::Bind() 遇到 ID=0 时会绑定黑色占位纹理，程序不会崩溃。
        g_matWood     = Material::CreateWood();
        g_matStone    = Material::CreateStone();
        g_matMetal    = Material::CreateMetal();
        g_matEmissive = Material::CreateEmissive();
        g_matFloor    = Material::CreateFloor();
        std::cout << "[Resources] Materials initialized (Wood/Stone/Metal/Emissive/Floor)" << std::endl;
        // ============================================================================
        // [插槽 0] 成员 B - 模型加载（待成员 B 填写）
        // ============================================================================
...
void RenderScene(Shader& shader) {
    ...
    // [插槽 2] 成员 C - 材质与纹理绑定
    // ============================================================================
    // 根据当前渲染对象选择对应材质并绑定到着色器。
    // 纹理单元分配约定（与 LightingPass 中的 shadowMap 不冲突）：
    //   GL_TEXTURE0 → material.diffuse  （漫反射贴图）
    //   GL_TEXTURE1 → material.specular （高光贴图）
    //   GL_TEXTURE2 → material.emissive （自发光贴图，可选）
    //   GL_TEXTURE3 → shadowMap         （由组长在 LightingPass 绑定，此处不动）
    //
    // 注意：Shadow Pass 阶段不需要绑定材质纹理（深度着色器不采样颜色），
    //       但调用无害，因为深度着色器会忽略未使用的 sampler。
    // ============================================================================

    // --- 地板（石料/混凝土）---
    if (g_matFloor.has_value()) {
        g_matFloor->Bind(shader);
        // 成员 B 在此处调用地板模型的 Draw，例如：
        // shader.setMat4("model", glm::mat4(1.0f));
        // g_floorModel->Draw(shader);
    }

    // --- 建筑墙体（石料）---
    if (g_matStone.has_value()) {
        g_matStone->Bind(shader);
        // 成员 B：g_buildingModel->Draw(shader);
    }

    // --- 木制道具（箱子、桌椅等）---
    if (g_matWood.has_value()) {
        g_matWood->Bind(shader);
        // 成员 B：g_propsModel->Draw(shader);
    }

    // --- 金属设备（机器、栏杆等）---
    if (g_matMetal.has_value()) {
        g_matMetal->Bind(shader);
        // 成员 B：g_metalModel->Draw(shader);
    }

    // --- 发光体（灯具、魔法阵等）---
    if (g_matEmissive.has_value()) {
        g_matEmissive->Bind(shader);
        // 成员 B：g_emissiveModel->Draw(shader);
    }
    // ============================================================================

    (void)shader; // 占位，成员 B 填入 Draw 调用后可删除此行
}
